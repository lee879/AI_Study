...
class GAM_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_channel_att=channel_shuffle0(x_channel_att,4) #last shuffle
        x = x * x_channel_att

        x_spatial_att = self.spatial_attention(x).sigmoid()
        x_spatial_att = channel_shuffle0(x_spatial_att, 4)  # last shuffle
        out = x * x_spatial_att
        # out=channel_shuffle0(out,4) #last shuffle
        return out

def channel_shuffle0(x, groups=2):
...
class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        # p = self.dr(self.act(self.bn(self.conv(x))))
        # return p
    def forward_fuse(self, x):

        # return self.dr(self.act(self.conv(x)))
...
class TransformerLayer(nn.Module):
    ...
    # remove layernorml for the better performer
    def forward(self, x):
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        p = x.flatten(2).unsqueeze(0).transpose(0, 3).squeeze(3)  #位置编码

        # return self.tr(p + self.linear(p)).unsqueeze(3).transpose(0, 3).reshape(b, self.c2, w, h)

        return self.tr(p).unsqueeze(3).transpose(0, 3).reshape(b, self.c2, w, h) #no study
    # def forward(self, x):
    #     if self.conv is not None:
    #         x = self.conv(x)
    #     b, _, w, h = x.shape
    #     p =
    #     return self.tr(x+self.linear(x)).reshape(b, self.c2, w, h)
